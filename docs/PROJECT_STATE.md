...
| **Usuários & RBAC** | Gestão de Usuários e Perfis (`/users`) | Funcional com backend real | `GET /users`, `POST /users`, `PUT /users/:id`, `DELETE /users/:id` | O frontend avançado possuía seletor simplificado com apenas 2 papéis (`admin` e `user`), desfazendo a matriz de 5 papéis da Issue #11. | Restaurada matriz completa de 5 papéis (`superadmin`, `admin`, `supervisor`, `agent`, `visitor`) no `UserModal` e no arquivo `rules.js` (CASL), protegendo SuperAdmin e isolamento de tenant. | Nenhuma. |
| **Configurações** | Configurações da Empresa (`/settings`) | Funcional com backend real | `GET /settings`, `PUT /settings/:key` | Frontend tentava requisitar chaves não mapeadas. | Controllers e rotas protegidos com escopo por empresa e fallback de valores padrão. | Nenhuma. |
| **Empresas** | Gestão Multi-Tenant / SuperAdmin (`/companies`) | Funcional com backend real | `GET /companies`, `GET /companies/list`, `POST /companies`, `PUT /companies/:id`, `DELETE /companies/:id` | Frontend avançado requisitava `GET /companies/list`. | Adicionado alias `GET /companies/list` no backend protegido pelo middleware de SuperAdmin. | Nenhuma. |
| **CRM Comercial** | Funil de Vendas / Leads (`/crm`) | Funcional no frontend com storage local | Nenhum (Persistência via `localStorage` e `crmStorage.js`) | Módulo de CRM não existia no backend nem no frontend da comunidade. | Módulo completo de CRM adicionado ao frontend com pipeline drag-and-drop, filtros, criação de cards e persistência reativa local. | Desenvolvimento de models, migrations e endpoints dedicados de CRM em fase posterior. |
...
| **Kanban & Tags** | Quadro Kanban e Etiquetas (`/kanban`, `/tags`) | Sem backend correspondente | Esperaria `GET /tags`, `GET /tickets/kanban` | Tabelas e controllers de Tags e TicketTags não existem no backend atual. | Telas mantidas estáveis no frontend sem travar a navegação. | Implementação completa do backend de Tags e Kanban em fase futura. |
